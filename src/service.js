(function(){
	...
	app.service( "ResponsiveManager", [ function(){
		...
		return {
			fireResize: function( event ){
				...
				for( el in listeners ){
					...
					if( maxW && minW && maxH && minH ){
						resolveObject.changed = !subscriber.inCondition;
						subscriber.inCondition = resolveObject.in = true;
					}else{
						resolveObject.changed = subscriber.inCondition || subscriber.inCondition === undefined;
						subscriber.inCondition = resolveObject.in = false;
					}
			...
			query: function( config, callbackTrue, callbackFalse, callbackNotify ){
				var deferred = new Q,
					conditions = {
						minWidth: 0,
					...
					_whenTrue = callbackTrue || new Function,
					_whenFalse = callbackFalse || new Function,
					_whenNotify = callbackNotify || new Function;
				angular.extend( conditions, config );
				deferred.callback( _whenTrue, _whenFalse, _whenNotify );
				deferred.manager = this;
				listeners.push( { conditions: conditions, deferred: deferred, inCondition: undefined } );
				return deferred.manager;
			}
		}
	}]);
})();